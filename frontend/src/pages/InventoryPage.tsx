...
export const InventoryPage: React.FC = () => {
  ...
  const getConditionBadge = (condition: string) => {
    switch (condition) {
      case 'good':
        return <span className="px-2 py-0.5 text-xs font-medium rounded-full bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300">Baik</span>;
      case 'fair':
        return <span className="px-2 py-0.5 text-xs font-medium rounded-full bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300">Cukup</span>;
      case 'damaged':
        return <span className="px-2 py-0.5 text-xs font-medium rounded-full bg-rose-100 text-rose-800 dark:bg-rose-900/40 dark:text-rose-300">Rusak</span>;
      case 'lost':
        return <span className="px-2 py-0.5 text-xs font-medium rounded-full bg-slate-100 text-slate-800 dark:bg-slate-800 dark:text-slate-300">Hilang</span>;
      default:
        return <span className="px-2 py-0.5 text-xs font-medium rounded-full bg-slate-100 text-slate-700">{condition}</span>;
    }
  };
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'approved':
      case 'borrowed':
        return <span className="px-2.5 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-300 flex items-center gap-1 w-fit"><Clock className="w-3 h-3" /> Dipinjam</span>;
      case 'returned':
        return <span className="px-2.5 py-1 text-xs font-medium rounded-full bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300 flex items-center gap-1 w-fit"><CheckCircle2 className="w-3 h-3" /> Kembali</span>;
      case 'overdue':
        return <span className="px-2.5 py-1 text-xs font-medium rounded-full bg-rose-100 text-rose-800 dark:bg-rose-900/40 dark:text-rose-300 flex items-center gap-1 w-fit"><AlertCircle className="w-3 h-3" /> Telat</span>;
      default:
    ...
    <div className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 text-sm border border-slate-300 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-900 text-slate-900 dark:text-white"
              />
            </div>
            <div>
              <select
          ...
          {isItemsLoading ? (
            ...
            <div className="py-12 text-center bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
              ...
              <p className="text-slate-600 dark:text-slate-400 font-medium">Belum ada barang inventaris</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {itemsData?.data?.map((item) => (
                <div
                  key={item.id}
                  className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 flex flex-col justify-between hover:shadow-sm transition"
                >
                  <div className="space-y-3">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        <h3 className="font-semibold text-slate-900 dark:text-white text-base mt-1">
                          {item.name}
                        </h3>
                    ...
                    <div className="pt-2 border-t border-slate-100 dark:border-slate-800 grid grid-cols-2 gap-2 text-xs">
                      ...
                      <div>
                        ...
                        <p className={`font-semibold ${item.available_quantity > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
                          {item.available_quantity} {item.unit}
                        </p>
                      </div>
                      {item.location && (
                        <div className="col-span-2 text-slate-500 flex items-center gap-1">
                          <MapPin className="w-3 h-3" /> Simpan di: <span className="text-slate-700 dark:text-slate-300 font-medium">{item.location}</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="pt-4 mt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-2">
                    <div>
                      {item.is_borrowable ? (
                        <span className="text-[11px] text-emerald-600 dark:text-emerald-400 font-medium bg-emerald-50 dark:bg-emerald-950/30 px-2 py-0.5 rounded">
                          Bisa Dipinjam
                        </span>
                      ) : (
                    ...
                    {!isResident && (
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="px-2.5 py-1 text-xs font-medium text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 rounded hover:bg-slate-200 dark:hover:bg-slate-700 transition"
                        >
                          Edit
                        </button>
                        <button
                          ...
                          className="px-2 py-1 text-xs font-medium text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/30 rounded transition"
                        >
                          Hapus
                        </button>
                      </div>
        ...
        <div className="space-y-4">
          ...
          {isBorrowingsLoading ? (
            ...
            <div className="py-12 text-center bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
              ...
              <p className="text-slate-600 dark:text-slate-400 font-medium">Belum ada transaksi peminjaman</p>
            ...
            <div className="overflow-x-auto bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl">
              <table className="w-full text-left border-collapse text-sm">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 text-slate-600 dark:text-slate-400 text-xs uppercase font-semibold">
                  ...
                  {borrowingsData?.data?.map((b) => (
                    <tr key={b.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition">
                      <td className="py-3 px-4">
                        <div className="font-medium text-slate-900 dark:text-white">{b.borrower_name}</div>
                        {b.borrower_phone && (
                          <div className="text-xs text-slate-500">{b.borrower_phone}</div>
                        )}
                      </td>
                      <td className="py-3 px-4">
                        <div className="font-medium text-slate-800 dark:text-slate-200">{b.item_name || 'Item'}</div>
      ...
      {isItemModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-xl max-w-lg w-full p-6 border border-slate-200 dark:border-slate-800 shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSaveItem} className="space-y-4 mt-4">
              ...
              <div className="flex items-center gap-2 pt-2">
                ...
                />
                <label htmlFor="is_borrowable" className="text-xs font-medium text-slate-700 dark:text-slate-300">
      ...
      {isBorrowModalOpen && targetItemForBorrow && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-xl max-w-md w-full p-6 border border-slate-200 dark:border-slate-800 shadow-xl">
            <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800">
              <div>
                <h3 className="font-semibold text-slate-900 dark:text-white">Catat Peminjaman Barang</h3>
      ...
      {isReturnModalOpen && selectedBorrowing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-xl max-w-md w-full p-6 border border-slate-200 dark:border-slate-800 shadow-xl">
            <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800">
              <h3 className="font-semibold text-slate-900 dark:text-white">Proses Pengembalian Barang</h3>
              <button
            ...
            <form onSubmit={handleSaveReturn} className="space-y-4 mt-4">
              <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg text-xs space-y-1">
                <div><span className="text-slate-500">Peminjam:</span> <span className="font-semibold text-slate-800 dark:text-slate-200">{selectedBorrowing.borrower_name}</span></div>
                <div><span className="text-slate-500">Barang:</span> <span className="font-semibold text-slate-800 dark:text-slate-200">{selectedBorrowing.item_name} ({selectedBorrowing.quantity} {selectedBorrowing.unit})</span></div>
              </div>
